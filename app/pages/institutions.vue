...
const { data: institutions } = await useFetch<CmsResult<Institution>>(
  "/api/institutions",
  { key: "institutions" }
);
...
<template>
  <div class="">
    <aside class="flex gap-4 flex-row flex-wrap  mb-8">
      <UInput v-model="filters.name" placeholder="Search instiution by name" size="md" class="basis-60 flex-none" />

      <USelectMenu 
        v-model="filters.country" :items="countries" placeholder="Filter by country" size="md"
      ...
      <USelectMenu 
        v-model="filters.course_level" :items="course_levels" placeholder="Filter by course level" size="md"
        class="basis-1/4">
      ...
      <UButton color="secondary" variant="solid" class="cursor-pointer" @click="resetFilters">Reset Filters
      </UButton>
    </aside>
    <main class="flex-1 grow">
      <div class="flex justify-between border-b-2 mb-4">
        <h1>Institutions</h1>
        <span class="text-sm font-light">{{ filteredInstitutions?.length }} institutions</span>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <UCard v-for="institution in filteredInstitutions" :key="institution.id">
          <template #header>
            {{ institution.name }}
          </template>
